...
def self_citation(destnodes,edges,jsons,size): #parameters: edge,jsons
    self_cite = [[0 for x in range(2)] for y in range(len(destnodes))] #initializes matrix
    innerindex = 0
    for key in destnodes:
        ...
        self_cite[innerindex][0] = match
        self_cite[innerindex][1] = count
...
def sum_citation(destnodes,edge,size):
    sum_cite = [[0 for x in range(2)] for y in range(len(destnodes))]
    innerindex = 0
    for key in destnodes:
        match = userdict[key]
        count = 0
        for row in range(size):
            if match == edge[row][0]:
                count = count + edge[row][2]
        sum_cite[innerindex][0]=match
        sum_cite[innerindex][1]=count
        innerindex = innerindex + 1
    return sum_cite
...
def self_citation_ratio(nodedict, self_cite, sum_cite):
    size = len(self_cite)
    self_cite_ratio = [[0 for x in range(2)] for y in range(size)] #initializes matrix
    innerindex = 0
    for row in range(size):
        if self_cite[row][0] == sum_cite[row][0]:
            ratio = float(self_cite[row][1]) / float(sum_cite[row][1])
            self_cite_ratio[innerindex][0] = nodedict[self_cite[row][0]]
            self_cite_ratio[innerindex][1] = ratio
            innerindex = innerindex + 1
    ...
    for item in OutDegV:
       for row in range(size):
           if item.GetVal1() == self_cite[row][0]:
               self_cite_ratio[innerindex][0] = item.GetVal1()
               self_cite_ratio[innerindex][1] = float(self_cite[row][1]) / float(item.GetVal2())
               innerindex = innerindex + 1
    return self_cite_ratio
